...
function resolve(context, path) {
  ...
  if (index !== -1) {
    ...
    // console.log("Cut: ", cut)
    // console.log("Reoslving: ", path.substr(0, cut), path.substr(cut + 1))
    // console.log("Resolution: ", resolve(context[path.substr(0, index)], path.substr(index + 1)))
      ...
      if (token[0] === 'static') {
        ...
        if (tag === 'partial') {
          ...
          function bump() {
            ...
            // if content has no periods, this just returns context[content].
            // otherwise, it will drill down into context object by object
            var item = resolve(context, content)
            ...
            if (tag === 'utag' || tag === 'etag') {
              // try {
              item = item.toString()
              // } catch (err) { console.log(context, '->', content, '=', item); throw err }
              if (tag === 'etag')
                item = escape(item)
