...
function resetCache(local) {
  ...
  fs.readdir(path.join(SETTINGS.root, local), function(err, files) {
    if (err) {
      throw err;
    }
    files.forEach(function(file) {
      var sublocal = path.join(local, file);
      var fullpath = path.join(SETTINGS.root, sublocal);
