...
function dottedEval(context, expression) {
  ...
  if (expression == '.') return context['.']; // for those embedded sections (silly Mustache spec, should be _)
...
function expressionEval(context, expression) {
  ...
  if (m = expression.match(/^(\S+)\s*([!=<>]+)\s*(\S+)$/)) {
    ...
    // we get the binding by doing a plain old dottedEval but with everything short of the last period
    var binding_str = m[1].replace(/(^|\.)[^.]+$/, '');
    var binding = dottedEval(context, binding_str) || null;
