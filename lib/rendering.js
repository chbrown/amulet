      ...
      if (node.t === 'raw') {
        ...
        (function bump() {
          ...
          if (node.t === 'unescaped') {
            ...
            if (enabled) {
              var subsection_name = node.subsection || '_';
              function sectionRender(item, context, callback) {
                var old_context_center;
                if (typeof(item) !== 'object') {
                  // for strings, numbers, booleans, etc.
                  var boxed_item = {};
                  boxed_item[subsection_name] = item;
                  item = boxed_item;
                }
                else {
                  old_context_center = context[subsection_name];
                  context[subsection_name] = item;
                }
                // console.log(item);
                ...
                return renderer.renderTokens(node.block, [], item, function() {
                  ...
                  if (old_context_center !== undefined) { context[subsection_name] = old_context_center; }
                  callback();subsection_name
                });
              }
