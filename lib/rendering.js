...
Renderer.prototype.renderTokens = function(tokens, context, callback) {
  ...
  (function next() {
    try {
      ...
      if (node === null) {
        // this might have been a node/token that was optimized
        // out by a parsing post-processor, so just ignore it
        return next();
      }
          ...
          if (node.t === 'unescaped') {
            ...
            if (enabled) {
              var sectionRender = function(local_item, context, callback) {
                // for an object with properties, we DO NOT expose it as _
                //   which causes problems with circularity.
                //   to comply with mu, and for the sake of concision,
                //   we expose its properties nakedly
                ...
                // return renderer.renderTokens(node.block, item, callback);
              ...
              if (item instanceof Array) {
                ...
                (function next2() {
                  var top = item[j++];
                  if (top) {
                    // console.log('sectionRenderArray', top, typeof top);
                ...
                // console.log('sectionRender', item, context, next);
